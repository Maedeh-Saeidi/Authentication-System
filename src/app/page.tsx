...
export default function Home() {
  ...
  const handleLogin = async () => {
    ...
    if (result.data?.data) {
      ...
      saveUser(user);
      console.log(result.data.data?.results?.[0], "result");
      // redirecting to dashb
    }
  };
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gray-100">
      <div className="bg-white p-6 rounded-xl shadow-md w-full max-w-sm">
        ...
        />
        <Button onClick={handleLogin} loading={isLoading || isFetching} disabled={isFetching}>
          Login
        </Button>
        {isError && <p className="text-red-500 text-sm mt-2">Something went wrong</p>}
      </div>
    </div>
  );
}
